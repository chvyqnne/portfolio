...
const colorMap: Record<string, string> = {
	...
	'coffee order': 'bg-teal-50 border-teal-200 text-teal-700',
};
			...
			{items.map(({ label, content, emoji }) => {
				...
				return (
					<motion.div
						...
						whileHover={{ y: -6 }}
					>
						<p className='uppercase text-xs font-semibold tracking-wide flex items-center gap-1'>
							{emoji ? <img
								alt={emoji}
								className='w-5 h-5 emoji-shake'
								src={`https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/svg/${emoji.codePointAt(0)?.toString(16)}.svg`}
							/> : null}
							{label}
						</p>
						<p className='text-sm mt-1 font-dmsans leading-snug'>{content}</p>
					</motion.div>
				);
			})}
		</div>
